...
import { ThirdStep } from "@components/pages/signup/forms/ThirdStep";
...
import { MyProfile } from "../queries/useMyProfileQuery";
...
export const EditProfilePopup = NiceModal.create(
  ({
    ...
    onOk: () => any;
    initialProfile: MyProfile["profile"];
  }) => {
    const { register, watch, handleSubmit, ...form } = useForm({
      defaultValues: {
    ...
    const onSubmit = useCallback(
      () =>
        handleSubmit(async ({ introduction, name }) => {
          await mutateAsync({ introduction, name, imageFile }).catch(() => {
            alert("알 수 없는 오류가 발생했습니다.");
          });
          modal.remove();
        })(),
      [handleSubmit, imageFile, modal, mutateAsync]
    );
